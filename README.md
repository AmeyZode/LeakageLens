...
Make sure Python dependencies and Frontend Node modules are installed:

```bash
# Install Python backend & dev dependencies
pip install -e ".[dev]"
...
### 2. Option A: Quick Start (Backend + Frontend Dev Servers)
Run both the FastAPI backend and Vite React web application simultaneously:
...
### 3. Option B: Running Services Separately
...
### 4. Running the CLI Auditor
Audit ML projects directly from your terminal:

```bash
# Run audit on sample projects (default Rich terminal report)
python3 -m leakagelens.main audit sample_projects

# Output report in JSON format
python3 -m leakagelens.main audit sample_projects --format json

# Output report in Markdown format to a file
python3 -m leakagelens.main audit sample_projects --format markdown --output audit_report.md
```
